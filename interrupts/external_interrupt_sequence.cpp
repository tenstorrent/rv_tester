...
void external_interrupt_sequence::drive_interrupt(){
   ...
   unsigned interrupt_num  = 1;//FIXME (rng1() % (FLAGS_imsic_intr_threshold )) ; 
   unsigned interrupt_file = 0;//FIXME (rng1() % (3 )) ; //gen either machine supervisor or hypervisor file
   unsigned interrupt_hart = 0;//FIXME (rng1() % (FLAGS_imsic_hart_threshold )) ; // sel rand hart
   unsigned vs_id          = 0;//FIXME (rng1() % (FLAGS_imsic_vs_intr_threshold )) ; //sel vs id
