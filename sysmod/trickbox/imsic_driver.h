...
DECLARE_bool(random_aclint_forcesync);//, false, "Drive random interrups");
DECLARE_int32(max_intr_count);
DECLARE_int32(max_acsync_count);
...
public:
  ...
  virtual void driveAclintForceSync()
  {
    cvm::log(cvm::HIGH,"[Trickbox] Driving ACLINT force sync message\n");
...
protected:
  ...
  void processDelayedRandomInterrupts()
  {
    ...
    //RANDOM imsic_intr
    if(FLAGS_random_aclint_forcesync && (acsync_count <= (int)FLAGS_max_acsync_count)){
      if(timer_ >= timer_rand_acsync){
        uint32_t rand_num =  (rng() % (10 - 5 + 1)) + 5;
        timer_rand_acsync = timer_ +(rand_num*timer_advance);
	      cvm::log(cvm::HIGH, "[Trickbox] Next random aclint force sync will be sent at  {}  \n", timer_rand_acsync);
        driveAclintForceSync();
        if(limit_acsync){
          acsync_count++;
        }
      }
...
private:
 //unsigned numMSIs_ = 6;
  ...
  uint64_t timer_rand_acsync  = 400;
