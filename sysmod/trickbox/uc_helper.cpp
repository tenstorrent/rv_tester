...
uc_helper::update_mem_model(uc_helper::trickbox_mem_req_t& i) {
  ...
  if ((!cvm::registry::messenger.call<whisperClient<uint64_t>::whisperPokeMemRPC>(cvm::topology::get_from_hierarchy("TOP.PLATFORM.WHISPER_CLIENT", 0), hart, 0, 'm', uc_helper_base,8, poke_data, false, valid)|| !valid) && FLAGS_whisper_client_check) {
 		 ...
 		 const strb_t&)
{
  ...
  if (addr == uc_helper_base) {
    ...
    for (size_t i = 0; i < tx_size; i++) {
      ...
      bool valid;
      if ((!cvm::registry::messenger.call<whisperClient<uint64_t>::whisperPokeMemRPC>(cvm::topology::get_from_hierarchy("TOP.PLATFORM.WHISPER_CLIENT", 0), 0/*hart*/, 0, 'm', tx_addr + i, 1, poke_data, false, valid)) && FLAGS_whisper_client_check) { //Poke same data to whisper memory
       cvm::log(cvm::ERROR, "Error: Failed to poke whisper memory\n");
       return;
      }
    ...
    bool valid;
    if ((!cvm::registry::messenger.call<whisperClient<uint64_t>::whisperPokeMemRPC>(cvm::topology::get_from_hierarchy("TOP.PLATFORM.WHISPER_CLIENT", 0), 0/*hart*/, 0, 'm', uc_helper_base,1, poke_data, false, valid)) && FLAGS_whisper_client_check) {
      cvm::log(cvm::ERROR, "Error: Failed to poke whisper memory\n");
      return;
    }
    return;
