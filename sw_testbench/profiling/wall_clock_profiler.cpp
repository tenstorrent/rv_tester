// Modified version of https://github.com/jasonrohrer/wallClockProfiler
...
char *stringsDuplicate(char** inStrings, int size) {
    int len = 0;
    for (int i = 0; i < size; ++i) {
        len += strlen(inStrings[i]);
    }
    char *returnBuffer = new char[len + 1];
    int pos = 0;
    for (int i = 0; i < size; ++i) {
        strcpy(&returnBuffer[pos], inStrings[i]);
        pos += strlen(inStrings[i]);
    }
    return returnBuffer;
}
...
int main( int inNumArgs, char **inArgs ) {
    ...
    char *progArgs = stringsDuplicate( &inArgs[3], inNumArgs - 3);
