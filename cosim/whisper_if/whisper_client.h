...
class whisperClient {
  ...
  public:
    ...
    virtual bool whisperStep(int hart, uint64_t time, uint64_t instrTag, uint64_t& pc, uint32_t& instruction, unsigned& changeCount, char* buffer, unsigned bufferSize, uint32_t& privMode, uint32_t& fpFlags, bool& hasTrap, bool& hasStop) = 0;
    ...
    virtual bool whisperPoke(int hart, char resource, uint64_t addr, uint64_t value, bool& valid) = 0;
