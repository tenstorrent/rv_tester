...
- `bridge.h` / `bridge.cpp` — The `bridge` class. `bridge.cpp` is the bulk of the logic: the `process_dut_*` event handlers (instruction retire, CSR HW updates, MCM read/insert/bypass/write/ifetch/ievict, interrupts, NMIs, timers, debug), the step/compare pipeline, CSR read/modify/mask helpers, interrupt defer/poke/peek machinery, translation checks, and the resynchronization heuristics.
- `bridge_base.h` — Abstract base declaring the `process_dut_*` virtual interface that `dut_if` components call into.
- `bridge_if.h` — The data contract between layers: `dut <— rv_instr_t —> bridge <— whisper_state_t —> whisper`. Defines `rv_instr_t` (a retired instruction with its PC/GPR/FPR/VR/CSR writes and memory accesses), `whisper_state_t` (one ISS step + state change), and the per-resource structs (`pc_t`, `gpr_t`, `csr_t`, `mem_t`, `rv_intr_t`, `rv_nmi_t`, `rv_debug_t`, ...).
- `bridge_params.h` — Compile-time parameters and constants.
- `bridge_plusargs.h` — Runtime plusarg declarations (e.g. logging toggles).
...
The bridge maintains maps for special CSR treatment — hypervisor-masked CSRs and their masked bit-fields (gated by `misa.H`), the full hypervisor CSR set, and CSRs that may be peeked/resynced for interrupts. These tables let it correctly mask, compare, or resync CSR state depending on enabled extensions.

`misa.H` save/restore: in the DUT, when `misa.H` is cleared to zero the hypervisor-related CSRs are saved and then restored when `misa.H` goes back to one. To keep the ISS consistent with this behavior, `rv_tester` saves those hypervisor CSRs in the CoreArchChecker when `misa.H` becomes zero and restores them when `misa.H` is set again.
