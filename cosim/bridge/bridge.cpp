...
bridge::~bridge() {}
  ...
  poke_mem(id_, 0, memmap_.at("boot").base + boot_num_harts_offset,    4, FLAGS_num_harts);
  poke_mem(id_, 0, memmap_.at("boot").base + boot_hart_sync_en_offset, 4, FLAGS_hart_sync_en);

  if(FLAGS_enable_sp_init){ //only poke num ways when sp_init is required
    uint64_t poke_data = uint64_t(FLAGS_enable_sp_init);
    poke_mem(0, 0, memmap_.at("boot").base + boot_sp_init_offset, 8, poke_data);
    poke_data = uint64_t(FLAGS_num_sp_ways);
    poke_mem(0, 0, memmap_.at("boot").base + boot_sp_ways_offset, 8, poke_data);
  }
  if (FLAGS_matp_swid) {
    poke_mem(0, 0, memmap_.at("boot").base + boot_matp_swid_offset, 8, uint64_t(FLAGS_matp_swid));
  }
  if (FLAGS_num_sc_enabled_ways) {
    poke_mem(0, 0, memmap_.at("boot").base + boot_sc_enabled_ways_offset, 8, uint64_t(FLAGS_num_sc_enabled_ways));
  }
...
void bridge::csr_init() {
  ...
  for (const auto& csr_: csrs) {
    ...
    if (csr.nonzero_reset) {
      peek_csr(id_, csr.addr, data, mask, poke_mask, read_mask);
  ...
  // CSR FECFG2
  peek_csr(id_, C_FECFG2, data, mask, poke_mask, read_mask);
...
void bridge::pre_step_debug_poke(hart_id_t hart, const rv_instr_t& instr) {
  ...
  poke_mem(hart, 0, instr.pc.pc_rdata, 4 /*Size*/, opcode);
  return;
}
...
void bridge::pre_step_interrupt_poke(hart_id_t hart, const rv_instr_t& d, whisper_state_t& w) {
  ...
  if (d.intr && !w_intr && !FLAGS_cosim_resynch) {
    ...
    if ((prev_hw_mip_[d.icause]) ||
      ...
      bridge_log(cvm::MEDIUM, "<{}> Timing sensitive mismatch: Resynch and keep going. cause: {})\n", w.time, d.icause);
      if (d.icause != 9) {
        std::bitset<64> mip = 0;
      ...
      if (deferred_intr_) {
        ...
        bridge_log(cvm::MEDIUM, "<{}> Timing sensitive mismatch. Undefer all interrupts.\n", w.time);
      }
    }
    return;
  }
  ...
  if (d.icause != w_cause || (d.virt_mode != w_virt_mode)) {
      bridge_log(cvm::MEDIUM, "<{}> DUT vs Whisper interrupt cause mismatch or Virtual mode mismatch [{},{}] age [{},{}]  DVirt_mode:{} WVirt_mode:{}\n",
    ...
    if (prev_hw_mip_[d.icause]) {
      bridge_log(cvm::MEDIUM, "<{}> cause: [{}] (Timing sensitive mismatch: Resynch and keep going)\n",w.time, d.icause);
      if(d.icause != 9)
      ...
      bridge_log(cvm::MEDIUM, "<{}> cause: [{}] (Defer and check)\n", w.time, w_cause);
  ...
  if (deferred_intr_) {
    defer_interrupt(hart, w.time, 0);
    deferred_intr_ = false;
    bridge_log(cvm::MEDIUM, "<{}> Undefer all interrupts\n", w.time);
  }
  ...
  step(hart, w);
  bridge_log(cvm::MEDIUM, "<{}> Whisper Step #{}: Extra step due to interrupt\n", w.time, step_);
}
...
void bridge::post_step_interrupt_check(hart_id_t hart, const rv_instr_t& d, const whisper_state_t& w) {
  ...
  if (!d.intr && !w_.intr && (w.disasm.find("csr") != std::string::npos)) {
    ...
    if (check_and_defer_interrupt(hart, d.cycle, mip)) {
      bridge_log(cvm::MEDIUM, "<{}> Timing sensitive case: deferring on interrupt x csr instr: {}\n", w.time, w.disasm);
      return;
    }
  }
  ...
  if (resynch_icause_) {
    ...
    bridge_log(cvm::MEDIUM, "<{}> Poking mip de assertion due to resynch in previous step {} \n", d.cycle, resynch_mip.to_ullong());
...
void bridge::post_step_satp_write_poke(hart_id_t hart, const rv_instr_t& d, const whisper_state_t& w) {
  ...
  if (w.disasm.find("satp") != std::string::npos) {
    for (auto& c : w_.csr) {
      if (c.csr_addr == SATP) {
        ...
        bool valid = false;
        if ((!cvm::registry::messenger.call<whisperClient<uint64_t>::whisperPokeRPC>(cvm::topology::get_from_hierarchy("TOP.PLATFORM.WHISPER_CLIENT", 0), hart, d.cycle, 'c', SATP, satp_, false, false, valid)) && FLAGS_whisper_client_check) {
  ...
  if (w.disasm.find("sfence.vma") != std::string::npos) {
    ...
    bool valid = false;
    if ((!cvm::registry::messenger.call<whisperClient<uint64_t>::whisperPokeRPC>(cvm::topology::get_from_hierarchy("TOP.PLATFORM.WHISPER_CLIENT", 0), hart, w.time, 'c', SATP, new_satp_, false, false, valid)) && FLAGS_whisper_client_check) {
  ...
  for (auto & c : d.csr) {
    ...
    if (FLAGS_csr_rd_check) {
      ...
      if (patch_mode_ != NO_PATCH) {
        ...
        peek_csr(hart, c.csr_addr, w_data, w_mask, w_poke_mask, w_read_mask);
      ...
      if (FLAGS_csr_rd_check){
        if (!is_indirect_reg(w.disasm) || nmi_.valid){
          ...
          if (w.address >= PMPADDR0 && w.address < PMPADDR16) {
            ...
            peek_csr(hart, PMPADDR0+pmp_cfg_reg, pmpcfg, mask, reset, read_mask);
...
bool bridge::disable_pa_check_vec(hart_id_t hart) {
  ...
  peek_csr(hart, VTYPE, data, mask, poke_mask, read_mask);
  ...
  peek_csr(hart, VL, data, mask, poke_mask, read_mask);
  vl = data & mask;
  ...
  if (debug_mode_) {
    ...
    if (cosim_util::is_csr_opcode(d.opcode, csr_addr) && csr_addr == DCSR && nmi_poke_in_debug_mode_) {
      ...
      if ((iss_val & ~dut_val) == 0b1000) { //
        bridge_log(cvm::MEDIUM, "<{}> Resynch: Reason=[nmi poke in debug mode]\n", d.cycle);
        return true;
      }
    }
  }
...
bool bridge::intr_csrs_mismatch(const hart_id_t& hart, const std::string& instr, std::string& , std::string& dut, std::string& iss, const uint64_t cycle, const rv_instr_t& d, const whisper_state_t& ) {
  ...
  if (FLAGS_mip_resynch &&
    ...
    bridge_log(cvm::MEDIUM, "<{}> Resynch: Reason=[mip_mismatch] [age={}]\n", cycle, hw_mip_age_);
    return true;
    }
  }
  if (FLAGS_topi_resynch && (csr_addr == MTOPI || csr_addr == STOPI || csr_addr == VSTOPI)) {
    if (hw_mip_age_ < FLAGS_mip_resynch_threshold || msi_.size() != 0) {
      bridge_log(cvm::MEDIUM, "<{}> Resynch: Reason=[topi_mismatch]\n", cycle);
      return true;
    }
  }
...
void bridge::resynch(hart_id_t hart, const rv_instr_t& d) {
  ...
  if (d.mem_write.valid) {
    ...
    poke_mem(hart, d.cycle, pa, d.mem_write.size, d.mem_write.data);
  }
  ...
  if (debug_mode_ || (m.pa>=sep_base_ && ((m.pa + m.size) < sep_end_))) {
    if (m.v_ext) {
      ...
      for (size_t i = 0; i < num_chunks && i < data_chunks.size(); ++i) {
        ...
        if (chunk_size > 0) {
          poke_mem(hart, m.cycle, chunk_addr, chunk_size, data_chunks[i]);
        }
      }
    } else {
      poke_mem(hart, m.cycle, m.pa, m.size, m.data);
    }  
  }
...
void bridge::process_dut_timer(hart_id_t hart, rv_intr_t& i) {
  ...
  if (FLAGS_poke_mip_timer) {
    ...
    poke_csr(hart, i.cycle, time_.address, i.mtime);
...
void bridge::process_imsic_msi(hart_id_t hart, const mem_t& m) {
  ...
  poke_mem(hart, m.cycle, m.pa, 4, m.data);
...
void bridge::poke_csr(hart_id_t hart, uint64_t cycle, uint64_t csr_addr, uint64_t csr_value) {
  bool valid;
  if ((!cvm::registry::messenger.call<whisperClient<uint64_t>::whisperPokeRPC>(cvm::topology::get_from_hierarchy("TOP.PLATFORM.WHISPER_CLIENT", 0), hart, cycle, 'c', csr_addr, csr_value, false, false, valid)) && FLAGS_whisper_client_check) {
    error("Hart {}: Failed to poke CSR addr: {:#x}\n", hart, csr_addr);
    return;
  }
}

void bridge::peek_csr(hart_id_t hart, uint64_t csr_addr, uint64_t& csr_value, uint64_t& csr_w_mask, uint64_t& csr_w_poke_mask, uint64_t& csr_w_read_mask) {
  bool valid;
  if ((!cvm::registry::messenger.call<whisperClient<uint64_t>::whisperPeekCsrRPC>(cvm::topology::get_from_hierarchy("TOP.PLATFORM.WHISPER_CLIENT", 0), hart, csr_addr, csr_value, csr_w_mask, csr_w_poke_mask, csr_w_read_mask, valid)) && FLAGS_whisper_client_check) {
    error("Hart {}: Failed to peek csr: {:#x} \n", hart, csr_addr);
  }
}

void bridge::peek_csr(hart_id_t hart, uint64_t csr_addr, uint64_t& csr_value) {
  bool valid;
  if ((!cvm::registry::messenger.call<whisperClient<uint64_t>::whisperPeekRPC>(cvm::topology::get_from_hierarchy("TOP.PLATFORM.WHISPER_CLIENT", 0), hart, 'c', csr_addr, csr_value, valid)) && FLAGS_whisper_client_check) {
    error("Hart {}: Failed to peek {}\n", hart, csr_addr);
    return;
  }
}
...
void bridge::peek_mip(hart_id_t hart, uint64_t time, std::bitset<64>& mip) {
  ...
  uint64_t w_mip;
  peek_csr(hart, MIP, w_mip);
  peek_csr(hart, MVIP, mvip_);
  mip_ = std::bitset<64>(w_mip); 
  mip = mip_;
  bridge_log(cvm::MEDIUM, "<{}> Whisper peek: mip: {:#x}, mvip: {:#x}\n", time, mip_.to_ullong(), mvip_);
}
...
uint64_t bridge::modify_csr_data(hart_id_t hart, uint64_t addr, uint64_t data, uint8_t priv) {
  ...
  for (size_t i = 0; i < 4; i++) {
    if ((addr == HSTATEEN0 + i) || (addr == SSTATEEN0 + i)) {
      ...
      peek_csr(hart, MSTATEEN0+i, mstateen, mask_iss, reset, read_mask);
      result &= mstateen;
      if ((addr == SSTATEEN0 + i) && priv == VS) {
        uint64_t hstateen, mask_iss, reset, read_mask;
        peek_csr(hart, HSTATEEN0+i, hstateen, mask_iss, reset, read_mask);
        result &= hstateen;
      }
      break;
    }
...
uint64_t bridge::modify_csr_mask(hart_id_t hart, uint64_t addr, uint64_t data, uint64_t mask) {
  ...
  if (addr >= PMPADDR0 && addr < PMPADDR16) {
    ...
    pmp_cfg_reg = ((i*8) / 64) * 2;
    pmp_cfg_index = (i*8) % 64;
    peek_csr(hart, PMPADDR0+pmp_cfg_reg, pmpcfg, mask_iss, reset, read_mask);
    if((pmpcfg >> (pmp_cfg_index + 4)) & 0x1) {
      result = result | 0x1ff;
    } else {
      result = result | 0x3ff;
  ...
  if (addr == HGATP) {
    ...
    if (!valid_mode) {
      ...
      uint64_t mode_mask = get_csr(id_, src_t::dut, HGATP)>>60 | mode;
  ...
  if (!shadow_csr) {
    if (auto it = csrs.find(addr); it != csrs.end() && it->second.shadow_csr) {
      ...
      if (src == src_t::dut) {
        ...
        peek_csr(hart, shadow_csr, data, mask, poke_mask, read_mask);
        alias_mask = get_csr_poke_mask(hart, shadow_csr);
      }
...
uint64_t bridge::get_csr_mask(hart_id_t hart, uint64_t addr) {
  uint64_t data, mask, poke_mask, read_mask;
  peek_csr(hart, addr, data, mask, poke_mask, read_mask);
...
void bridge::report_metrics() {
  ...
  for (auto& csr_ : csrs) {
    ...
    if((!hyp_enabled()) && (hypervisor_csr_map_.find(csr.addr) != hypervisor_csr_map_.end())) {
      ...
      peek_csr(id_, csr.addr, csr_data);
