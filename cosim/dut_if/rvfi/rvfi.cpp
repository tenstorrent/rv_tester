...
void rvfi::process(const rv_tester_transactions::cosim::m_mcmi_bypass<>& m_mcmi_bypass) {
  ...
  if (numones == consecutiveOnes) {
      ...
      if(m_mcmi_bypass.cbo || ((m.amo && m.amo_op == SC) && !sc_failed(m))) {
...
void rvfi::process(const rv_tester_transactions::cosim::m_mcmi_devict<>& m_mcmi_devict) {
  ...
  cvm::log(cvm::MEDIUM, "Remote Procedural Call to Whisper for mcm devict to addr : {:#x}\n",m_mcmi_devict.addr);
  bool valid = false;
...
void rvfi::process(const rv_tester_transactions::cosim::m_mcmi_flush<>& m_mcmi_flush) {
  ...
  cvm::log(cvm::MEDIUM, "Remote Procedural Calls to Whisper for mcm flush (combination of writeback + evict) to addr : {:#x}\n",m_mcmi_flush.addr);
  bool valid = false;
