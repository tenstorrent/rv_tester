...
public:
  ...
  virtual void process_steps(hart_id_t hart, uint64_t cycle, uint64_t order) = 0;
  virtual void process_dut_csr_hw_update(hart_id_t hart, csr_t& c) = 0;
  virtual void process_counter_overflow(csr_t& c) = 0;
  ...
  virtual void process_dut_mtip(hart_id_t hart, uint64_t cycle, bool mtip, bool intr_during_ucode) = 0;
