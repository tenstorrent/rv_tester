...
template <typename T>
void atop_arithmetic(const axi::data_t& read_data, axi::data_t& write_data, const axi::atop_operation operation, const axi::len_t& len) {
...
bool axi::check_error(addr_t addr, size_t& count) {
  ...
  count = (slverr_count_nonzero) ? slverr_count_val : (decerr_count_nonzero) ? decerr_count_val
                                                                             : 0;
...
void axi::atop_modify_write_data(const atop_t& atop, const data_t& read_data, data_t& write_data, const len_t& len) {
  if (atop.transaction == NON_ATOMIC)
    return;
  ...
  if (!bitwise) {
    ...
    case 1:
      if (sign)
        atop_arithmetic<std::int8_t>(read_data, write_data, op, len);
      else
        atop_arithmetic<std::uint8_t>(read_data, write_data, op, len);
      break;
    case 2:
      if (sign)
        atop_arithmetic<std::int16_t>(read_data, write_data, op, len);
      else
        atop_arithmetic<std::uint16_t>(read_data, write_data, op, len);
      break;
    case 4:
      if (sign)
        atop_arithmetic<std::int32_t>(read_data, write_data, op, len);
      else
        atop_arithmetic<std::uint32_t>(read_data, write_data, op, len);
      break;
    case 8:
      if (sign)
        atop_arithmetic<std::int64_t>(read_data, write_data, op, len);
      else
        atop_arithmetic<std::uint64_t>(read_data, write_data, op, len);
      break;
    default:
    ...
    for (addr_t n = 1; n <= burst_len; n++) {
      ...
      if (1) {
        ...
        if (a.w) {
          ...
          bool inject_decerr = last && error_en_ && decerr_list_.check_inject_error(addr, WRITE);

          // Always increment counters for addresses in error ranges (regardless of injection)
          if (last && error_en_ && slverr_list_.find(addr)) {
          ...
          if (last && error_en_ && decerr_list_.find(addr)) {
        ...
        if (!a.w || (a.atop.transaction != NON_ATOMIC && a.atop.transaction != ATOMIC_STORE)) {
          ...
          bool inject_decerr = last && error_en_ && decerr_list_.check_inject_error(addr, READ);
