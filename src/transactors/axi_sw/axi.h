...
public:
  ...
  struct atop_t {
    ...
    atop_t(std::uint8_t raw) : transaction(
                                   atop_transaction(
                                       raw >> 4 == 3 ? ((raw >> 4) + (raw & 1)) : raw >> 4)),
  ...
  struct a_t {
    ...
    a_t(const bool& w, const id_t& id, const addr_t& addr, const len_t& len, const sz_t& size, const burst_t& burst, const bool& lock,
        const cache_mem_attr_t& cache, const prot_t& prot, const qos_t& qos, const region_t& region, const atop_t& atop, const user_t& user) : w(w), id(id), addr(addr), len(len), size(size), burst(burst), lock(lock), cache(cache), prot(prot), qos(qos), region(region), atop(atop), user(user) {}
