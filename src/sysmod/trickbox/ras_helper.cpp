...
void ras_helper::read_dev(uint64_t addr, size_t length, data_t& data) {
...
void ras_helper::write(uint64_t addr, size_t, const data_t& data,
                       const strb_t&) {
  if (not has_addr(addr))
    return;
  uint64_t t_data = 0;
