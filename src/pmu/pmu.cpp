...
void pmu::process_core(const rv_tester_transactions::pmu_core::pmcounters_core<>& pmcounters) {
...
void pmu::process_sc(const rv_tester_transactions::pmu_sc::pmcounters_sc<>& pmcounters) {
  if (loc_ != pmcounters.location)
    return;
...
pmu::trigger_str_core(const rv_tester_transactions::pmu_core::pmcounters_core<>& pmcounters) {
  return pmcounters.perf_start ? "perf_start" : pmcounters.perf_end ? "perf_end"
...
pmu::trigger_str_sc(const rv_tester_transactions::pmu_sc::pmcounters_sc<>& pmcounters) {
  return pmcounters.perf_start_sc ? "perf_start" : pmcounters.perf_end_sc ? "perf_end"
...
void pmu::process(const rv_tester::terminate_called_fast&) {
...
bool pmu::is_within_range(double actual, double expected, int tolerance_perc, bool higher_is_better) {
...
void pmu::process_core(const rv_tester_transactions::pmu_core::hpmcounters_core<>& hpmcounters) {
...
void pmu::get_filter_events_and_sum(uint64_t event_id,
...
void pmu::process_core(const rv_tester_transactions::pmu_core::pmc_checker<>& pmc_checker) {
