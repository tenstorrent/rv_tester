...
axi_sw::axi_sw(const svScope& scope, sysmod* s, bool r_poll, const axi::data_width_t& data_width, const std::string& tag, const r_q_ptr_t& r_q_max, const r_q_ptr_t& r_q_ptr_max)
  : scope_(scope), r_poll_(r_poll), r_q_max_(r_q_max), r_q_ptr_max_(r_q_ptr_max), r_q_rptr_(0), r_q_wptr_(0) {

    axi_ = new axi(s, data_width, tag);
...
extern "C" {
  axi_sw* axi_sw_new(sysmod* s, std::uint8_t r_poll, axi::data_width_t data_width, const char* tag,
    axi_sw::r_q_ptr_t r_q_max, axi_sw::r_q_ptr_t r_q_ptr_max) {
    svScope scope = svGetScope();
    return new axi_sw(scope, s, r_poll, data_width, tag, r_q_max, r_q_ptr_max);
  }
