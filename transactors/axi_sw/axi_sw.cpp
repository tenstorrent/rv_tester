...
cvm::messenger::task<void> axi_sw<W,AW,AR,RQ>::process(const AW& aw) {
    ...
    co_await a(axi::a_t{true, aw.id, aw.addr, aw.len, aw.size, axi::burst_t(aw.burst), aw.lock != 0,axi::cache_mem_attr_t(0),aw.prot,aw.qos,aw.region, aw.atop,aw.user});
    r_resp();
    co_return;
}
...
cvm::messenger::task<void> axi_sw<W,AW,AR,RQ>::process(const AR& ar) {
    ...
    co_await a(axi::a_t{false, ar.id, ar.addr, ar.len, ar.size, axi::burst_t(ar.burst), ar.lock != 0,axi::cache_mem_attr_t(0),ar.prot,ar.qos,ar.region, 0,ar.user});
    r_resp();
    co_return;
}
