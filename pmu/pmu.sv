...
#(
  ...
  `RV_TESTER_TRANSACTIONS_PMU_OUTPUT_PORTS
   ...
   endgenerate
    ...
    assign pmcounterss[0].valid = !reset && perf_enabled && ((terminate && !terminate_1T) || (cycle_sync_en && (sync_cycles % period) == 0) || (instruction_sync_en && (((prev_sync_instructions % instructions) > nret) && ((sync_instructions % instructions) < nret))) || perf_start || perf_end);
    ...
    assign pmcounterss[0].data.page_walks_ls = pmcounter[PAGE_WALKS_LS];
    assign pmcounterss[0].data.page_walks_fe = pmcounter[PAGE_WALKS_FE];
    assign pmcounterss[0].data.page_walks_mmu_prefetch = pmcounter[PAGE_WALKS_MMU_PREFETCH];
    assign pmcounterss[0].data.page_walks_all = pmcounter[PAGE_WALKS_ALL];
    ...
    assign pmcounterss[0].data.ldst_masked_vec_nano = pmcounter[LDST_MASKED_VEC_NANO];
