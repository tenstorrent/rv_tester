...
public:
  typedef enum : size_t {
    // Event for each CPU cycle; maps to the mcycle CSR
    CPU_CYCLES,
    // Event for each retired instruction; maps to the minstret CSR
    INSTRUCTIONS,
    // The count will represent all the requests made to cache subsystem that
    // hit
    // in any level in cache. includes accesses made by Speculatively executed
    // instructions and hardware prefetchers, but does not include non-cacheable
    // requests
    CACHE_REFERENCES,
    ...
    // hardware
    // prefetchers, but does not include non-cacheable requests
    CACHE_MISSES,
    // Event for retired branch instructions. Branch instructions are all
    // control
    // instructions -- conditional and unconditional, direct and indirect.
    ...
    // target
    // predictor. This will not include mispredictions due to next-fetch
    // predictor.
    BRANCH_MISSES,
    ...
    // include
    // accesses to IO or noncacheable regions
    L1D_WRITE_ACCESS,
