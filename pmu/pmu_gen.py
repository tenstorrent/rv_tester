...
def create_sv_frag(events: List[Dict[Any, Any]], path="gen_events.sv", prefix="", skip_pre = False):
    ...
    pre = f"""
    //AUTOGENERATED -- NO TOUCH
    assign pmcounterss[0].valid = !reset && (|mhpm_write) && perf_enabled && (terminate || (cycle_sync_en && (sync_cycles % period) == 0) || (instruction_sync_en && (((prev_sync_instructions % instructions) > nret) && ((sync_instructions % instructions) < nret))) || perf_start || perf_end);
