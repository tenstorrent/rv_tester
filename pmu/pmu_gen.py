...
def create_sv_frag(events: List[Dict[Any, Any]], prefix="", path="gen_events.sv"):
    tab = "      "
...
def create_defines_frag(events: List[Dict[Any, Any]], prefix="", path="gen_defines.sv"):
    ...
    poslude = """        {prefix.upper()}EVENT_COUNT
    } {prefix}pmc_event_t;"""
    f = open(path, 'w')
    f.write(prelude)
    for event in events:
...
def create_yaml_frag(events: List[Dict[Any, Any]], path="gen_events.yaml"):
    tab = "    "
...
create_sv_frag(sc_data_dict, "sc_", "gen_sc_events.sv")
