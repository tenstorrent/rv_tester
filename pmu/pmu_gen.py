...
# Core pmc events
spec_id = 246
sha = "167d18a1e656378f0428c98bf25fcf137f1c6105"
url = f"https://aus-gitlab.local.tenstorrent.com/api/v4//projects/{spec_id}/repository/files/src%2Fcluster%2Fascalon%5Fuarch%2Fsrc%2Fcore%5Fpmc%2Ecsv?ref={sha}"
data_str = download_csv(url)
core_data_dict = []
core_events = data_str.split('\n')

# Sharedcache pmc events
sc_url = f"https://aus-gitlab.local.tenstorrent.com/api/v4//projects/{spec_id}/repository/files/src%2Fcluster%2Fascalon%5Fuarch%2Fsrc%2Fsharedcache%5Fpmc%2Ecsv?ref={sha}"
...
def create_sv_frag(events: List[Dict[Any, Any]], f, prefix="", skip_pre = False):
    tab = "      "
    ...
    assign pmcounterss[0].data.location = {pmcounter_width}'(location);
...
def create_yaml_frag(events: List[Dict[Any, Any]], f, mod="", skip_prelude = False):
    tab = "    "
    if not skip_prelude:
...
with open("gen_events.sv", "w") as f:
    create_sv_frag(core_data_dict, f, "",)
    create_sv_frag(sc_data_dict, f, "sc_", True)
...
with open("gen_events.yaml", "w") as f:
    create_yaml_frag(core_data_dict, f, "", False)
    create_yaml_frag(sc_data_dict, f, "sc", True)
