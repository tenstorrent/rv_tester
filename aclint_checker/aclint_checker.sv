...
#(
    ...
    import "DPI-C" function void AcPrintErrMsg(input longint unsigned clock, input int unsigned hart, input int unsigned msg_num);
    ...
    always_comb assert (~violation_forcesync) else AcPrintErrMsg(clocks, n, 0);
    end
    ...
    always_comb assert(~(cycles_in_fail_mtishouldbeOFF > 5)) else AcPrintErrMsg(clocks, asserti, 1);
    always_comb assert(~(cycles_in_fail_mtishouldbeON > 5)) else AcPrintErrMsg(clocks, asserti, 2);
    end
    endgenerate
    ...
    always_comb assert (mtip8_delay_counter < max_mtip8_delay) else AcPrintErrMsg(clocks, 0, 3);
