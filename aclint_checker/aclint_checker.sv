...
#(
    ...
    logic mtip8_delayed, smc_mtip8_delayed, mtip8_assert, smc_mtip8_assert;

    // Sequential logic for delay line
    always @(posedge rf_clk) begin
        if (dut_reset) begin
            mtip8_delayed <= 0;
            smc_mtip8_delayed <= 0;
            ...
            mtip8_delayed <= AcMtipi[8];
            smc_mtip8_delayed <= SmcMtipi;
            mtip8_assert <= AcMtipi[8] & ~mtip8_delayed;
            smc_mtip8_assert <= SmcMtipi & ~smc_mtip8_delayed;
            if(mtip8_assert & ~smc_mtip8_assert) mtip8_delay_counter <= mtip8_delay_counter + 1;
            else mtip8_delay_counter <= 0;
        end
    end
    // Assertion outside procedural block
    ...
    for (genvar n = 0; n < TOTAL_NBYPASSES; n++) begin
        ...
        assign ac_axi_writes[0].data.hart = vid[AcReqPkti.srcid];
